...
export default function Oracle() {
    ...
    // Every 5 seconds, check if the Oracle UtxO has changed
    useEffect(() => {
        getOracleNftUtxO();
    ...
    const getOracleNftUtxO = async () => {
        if (lucid && wAddr && oracleAddress) {
            ...
            if (!oracUtxO) return;
            // We filter the utxos containing our NFT(asset class)
    ...
    /**
     *  Because we are sending a utxo to a validator, we don't really need to satisfy any checks.
     */
